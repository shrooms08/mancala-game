...
func choose_strategic_move(possible_moves: Array) -> int:
	# Check for moves that land in AI's store(pit 6) for extra turn
...
func update_game_ui():
	...
	if vs_ai:
		player_1_score_label.text = "AI: " + str(pits[6])
		player_2_score_label.text = "You: " + str(pits[13])
	else:
		player_1_score_label.text = "Player 1: " + str(pits[6])
		player_2_score_label.text = "Player 2: " + str(pits[13])
	...
	for i in range(14):
		...
		if pit_node is TouchScreenButton:
			...
			if pit_node.has_signal("pressed"):
				...
				print("Connected pressed signal for pit", i)
			else:
...
func handle_pit_click(index):
	...
	# Record move time for Honeycomb tracking
...
func animate_stone_distribution(start_index):
	...
	# Create all animated stones at once
	var animated_stones = []
...
func animate_all_stones_together(stones, sequence, start_index):
	...
	for i in range(stones.size()):
		...
		# Calculate the path for this stone
...
func check_capture(last_index) -> int:
	...
	show_status_message("Capture! +" + str(captured_stones) + " stones", Color.ORANGE)
...
# Individual pit functions (connected via the editor)
