...
const grantXPSchema = zod_1.z.object({
    userPubkey: zod_1.z.string().min(32).max(44),
    amount: zod_1.z.number().positive(),
    reason: zod_1.z.string(),
    gameId: zod_1.z.string().optional(),
    timestamp: zod_1.z.string().optional()
});
const missionProgressSchema = zod_1.z.object({
    userPubkey: zod_1.z.string().min(32).max(44),
    missionId: zod_1.z.string(),
    progress: zod_1.z.number().nonnegative(),
    gameId: zod_1.z.string().optional(),
    timestamp: zod_1.z.string().optional()
});
const gameResultSchema = zod_1.z.object({
    ...
    gameMode: zod_1.z.enum(['PVP', 'PVE']),
    aiDifficulty: zod_1.z.enum(['Easy', 'Medium', 'Hard']).optional(),
    stats: zod_1.z.object({
        stonesCaptured: zod_1.z.number().nonnegative(),
        extraTurns: zod_1.z.number().nonnegative(),
        fastMoves: zod_1.z.number().nonnegative(),
        playTime: zod_1.z.number().nonnegative(),
        totalMoves: zod_1.z.number().nonnegative()
    }),
    timestamp: zod_1.z.string().optional()
});
...
// Get project status
gameRouter.get('/project', async (req, res) => {
    const requestId = `game_project_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    logger_1.logContext.api('Getting project status', { requestId });
    try {
        const state = (0, state_1.loadState)();
        ...
        logger_1.logContext.error('Failed to get project status', error, 'game');
        res.status(500).json({
            success: false,
...
// Grant XP to user
gameRouter.post('/grant-xp', async (req, res) => {
    const requestId = `grant_xp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    logger_1.logContext.api('Granting XP to user', {
        requestId,
        body: req.body
    });
    try {
        ...
        if (!parse.success) {
            logger_1.logContext.api('Invalid XP grant data', {
                requestId,
        ...
        if (!user) {
            logger_1.logContext.api('User not found for XP grant', {
                requestId,
                userPubkey
            });
        ...
        logger_1.logContext.api('XP granted successfully', {
            requestId,
            userPubkey,
            amount,
            reason,
        ...
        logger_1.logContext.error('Failed to grant XP', error, 'game');
        res.status(500).json({
            success: false,
...
gameRouter.post('/mission-progress', async (req, res) => {
    const requestId = `mission_progress_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    logger_1.logContext.api('Updating mission progress', {
        requestId,
        body: req.body
    });
    try {
        ...
        if (!parse.success) {
            logger_1.logContext.api('Invalid mission progress data', {
                requestId,
                errors: parse.error.issues
            });
            return res.status(400).json({
        ...
        const user = users[userPubkey];
        if (!user) {
            logger_1.logContext.api('User not found for mission progress', {
                requestId,
                userPubkey
            });
            return res.status(404).json({
        ...
        if (user.missions && user.missions[missionId]) {
            ...
            if (mission.completed && !wasCompleted) {
                ...
                user.stats.totalXP = newXP;
                user.stats.currentLevel = newLevel;
                logger_1.logContext.api('Mission completed and reward XP granted', {
                    requestId,
                    userPubkey,
                    missionId,
                    rewardXP,
            ...
            users[userPubkey] = user;
            saveUsersToState(users);
            logger_1.logContext.api('Mission progress updated successfully', {
                requestId,
                userPubkey,
                missionId,
                progress,
        ...
        logger_1.logContext.error('Failed to update mission progress', error, 'game');
        res.status(500).json({
            success: false,
...
// Record game result
gameRouter.post('/game-result', async (req, res) => {
    const requestId = `game_result_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    logger_1.logContext.api('Recording game result', {
        requestId,
        body: req.body
    });
    try {
        ...
        if (!parse.success) {
            logger_1.logContext.api('Invalid game result data', {
                requestId,
                errors: parse.error.issues
            });
            return res.status(400).json({
        ...
        const user = users[userPubkey];
        if (!user) {
            logger_1.logContext.api('User not found for game result', {
                requestId,
                userPubkey
            });
            return res.status(404).json({
        ...
        users[userPubkey] = user;
        saveUsersToState(users);
        logger_1.logContext.api('Game result recorded successfully', {
            requestId,
            userPubkey,
            gameId,
            result,
        ...
        logger_1.logContext.error('Failed to record game result', error, 'game');
        res.status(500).json({
            success: false,
...
gameRouter.get('/stats/:userPubkey', async (req, res) => {
    ...
    try {
        ...
        const user = users[userPubkey];
        if (!user) {
            logger_1.logContext.api('User not found for stats', {
                requestId,
                userPubkey
            });
            return res.status(404).json({
        ...
        logger_1.logContext.error('Failed to get user statistics', error, 'game');
        res.status(500).json({
            success: false,
